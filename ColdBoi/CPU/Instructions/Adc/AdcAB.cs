...
{
    public class AdcAB : Instruction
    {
        public const byte OPCODE = 0x88;
        public const string NAME = "adc";

        public AdcAB(Processor processor) : base(processor, OPCODE, 0, 4, NAME)
        {
        }

        public override void Execute(params byte[] operands)
        {
            var value = this.processor.Registers.BC.HigherByte + Convert.ToByte(this.processor.Registers.Carry.Value);
