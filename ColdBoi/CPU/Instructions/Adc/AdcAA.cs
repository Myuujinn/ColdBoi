...
{
    public class AdcAA : Instruction
    {
        public const byte OPCODE = 0x8f;
        public const string NAME = "adc";

        public AdcAA(Processor processor) : base(processor, OPCODE, 0, 4, NAME)
        {
        }

        public override void Execute(params byte[] operands)
        {
            var value = this.processor.Registers.AF.HigherByte + Convert.ToByte(this.processor.Registers.Carry.Value);
