...
{
    public class AdcAH : Instruction
    {
        public const byte OPCODE = 0x8c;
        public const string NAME = "add";

        public AdcAH(Processor processor) : base(processor, OPCODE, 0, 4, NAME)
        {
        }

        public override void Execute(params byte[] operands)
        {
            var value = this.processor.Registers.HL.HigherByte + Convert.ToByte(this.processor.Registers.Carry.Value);
