...
{
    public class AdcAC : Instruction
    {
        public const byte OPCODE = 0x89;
        public const string NAME = "adc";

        public AdcAC(Processor processor) : base(processor, OPCODE, 0, 4, NAME)
        {
        }

        public override void Execute(params byte[] operands)
        {
            var value = this.processor.Registers.BC.LowerByte + Convert.ToByte(this.processor.Registers.Carry.Value);
