...
{
    public class AddHlSp : Instruction
    {
        public const byte OPCODE = 0x39;
        public const string NAME = "add";

        public AddHlSp(Processor processor) : base(processor, OPCODE, 0, 8, NAME)
        {
        }

        public override void Execute(params byte[] operands)
        {
            var value = this.processor.Registers.SP.Value;
...
#if DEBUG
            Console.WriteLine($"{this.processor.Registers.PC.Value:X4}: {this.Name} hl, sp");
#endif
        }
    }
}